...
describe("resolveModelWithFallback", () => {
  ...
  describe("Step 3: Provider fallback chain", () => {
    ...
    test("cross-provider fuzzy match when preferred provider unavailable (librarian scenario)", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        fallbackChain: [
          { providers: [SUPPORTED_PROVIDERS.ZAI_CODING_PLAN], model: SUPPORTED_MODELS.GLM_5 },
    ...
    test("cross-provider match preserves variant from entry", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        fallbackChain: [
          { providers: [SUPPORTED_PROVIDERS.ZAI_CODING_PLAN], model: SUPPORTED_MODELS.GLM_5, variant: "high" },
        ],
  ...
  describe("Multi-entry fallbackChain", () => {
    test("resolves to claude-opus when OpenAI unavailable but Anthropic available (oracle scenario)", () => {
      ...
      const result = resolveModelWithFallback({
        fallbackChain: [
          { providers: [SUPPORTED_PROVIDERS.OPENAI, SUPPORTED_PROVIDERS.GITHUB_COPILOT, "opencode"], model: SUPPORTED_MODELS.GPT_5_4, variant: "high" },
          { providers: [SUPPORTED_PROVIDERS.ANTHROPIC, SUPPORTED_PROVIDERS.GITHUB_COPILOT, "opencode"], model: SUPPORTED_MODELS.CLAUDE_OPUS_4_7, variant: "max" },
        ],
        availableModels,
