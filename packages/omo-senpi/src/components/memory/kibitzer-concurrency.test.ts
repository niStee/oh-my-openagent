...
const key = Symbol.for("omo.kibitzer.judgeSlots")

afterEach(() => {
  Reflect.deleteProperty(globalThis, key)
})

describe("kibitzer judge concurrency", () => {
  test("holds at most two judge slots and releases them", async () => {
    const module = await import("./kibitzer-concurrency")
  ...
  test("a second module instance shares the process-global count", async () => {
    const module = await import("./kibitzer-concurrency")
    const release = module.tryAcquireJudgeSlot()
    const second = await import(`./kibitzer-concurrency.ts?cacheBust=${Date.now()}`)
