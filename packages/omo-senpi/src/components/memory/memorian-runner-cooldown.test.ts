...
import { callNudge, CANDIDATES, fixture, launchInput, nudgeOnce, roots, runnerOptions, scriptedSession } from "./memorian-runner.test-support"
...
describe("MemorianGateRunner nudge cooldown", () => {
  ...
  test("#given an active launch and later compaction #when the session reaches two accepts #then neither resets or overspends the budget but a new runner starts fresh", async () => {
    ...
    const runner = new MemorianGateRunner(options)
    const input = launchInput()
    ...
    expect(stub.created).toBe(2)
    expect((await new MemorianGateRunner(options).launch(input)).status).toBe("nudged")
  })
