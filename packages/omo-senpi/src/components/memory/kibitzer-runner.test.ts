...
import { CANDIDATE_PATH, fixture, launchInput, nudgeOnce, registrySnapshot, roots, runnerOptions, scriptedSession } from "./kibitzer-runner.test-support"
...
describe("KibitzerGateRunner", () => {
  ...
  test("#given no registry snapshot on the input #when the runner launches #then it warns, skips and creates no child session", async () => {
    ...
    expect(warnings).toEqual(["kibitzer gate registry snapshot unavailable"])
  })
  ...
  test("#given no quick category but another usable registry model #when the runner launches #then it warns, skips and never rides the beyond-category ladder", async () => {
    ...
    expect(result.status).toBe("skipped")
    expect(stub.created).toBe(0)
    expect(warnings).toEqual(["kibitzer gate quick category unavailable"])
  })
  ...
  test("#given a child turn that ends with an error #when the runner launches #then the failure names the child cause", async () => {
    ...
    const runner = new KibitzerGateRunner(runnerOptions(identityPaths, { createSession: failing.createSession }))
  ...
  test("#given an inlined launch #when the child session is created #then the judge prompt carries the inputs inline with a bare envelope", async () => {
    ...
    // it needs no file access and the ancestry wrapper never wraps the kibitzer persona.
    const { identityPaths } = await fixture()
    const stub = scriptedSession(nudgeOnce)
    ...
    const runner = new KibitzerGateRunner(runnerOptions(identityPaths, { createSession }))
    ...
    expect(prompt).toContain("<kibitzer-input>")
    ...
    // and the child session runs the kibitzer persona as its system prompt with only the nudge tool
    expect(captured?.resourceLoader?.getSystemPrompt()).toBe(loadKibitzerPersona())
