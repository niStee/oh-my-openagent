...
export interface MemorianHooksOptions {
  readonly trigger: Pick<MemorianTrigger, "onPrompt" | "onToolCall" | "onSettled">
  readonly delivery: Pick<MemorianDelivery, "onToolResult" | "markRunning" | "markSettled">
...
export function registerMemorianHooks(pi: SenpiExtensionAPI, options: MemorianHooksOptions): void {
  pi.on("before_agent_start", (payload, eventCtx) => {
    try {
      ...
      options.logger?.warn("omo-senpi memorian prompt trigger failed", { error: describe(error) })
    }
    return undefined
  })
  ...
  pi.on("agent_settled", (_payload, eventCtx) => {
    try {
      ...
      options.logger?.warn("omo-senpi memorian settle hook failed", { error: describe(error) })
    }
    return undefined
  })
}
