...
import { callNudge, fixture, registrySnapshot, roots, runnerOptions, scriptedSession, SESSION_ID } from "./memorian-runner.test-support"
...
test("#given A and B accepted through the trigger and C skipped for cooldown #when the window expires #then identical C creates a judge and judged C stays suppressed", async () => {
  ...
  const context = createMemoryIdentityContext({
    identity: "memorian-agent", identityPaths,
    binding: createMemoryBinding({ identity: "memorian-agent", repoPath: identityPaths.repo, boundAt: 0 }),
  })
