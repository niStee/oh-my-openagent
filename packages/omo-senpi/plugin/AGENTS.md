...
| Path | Purpose |
|------|---------|
| `package.json` | Pi manifest: `pi.extensions = ["./extensions/omo.js"]`, deliberately NO `pi.skills` (bundled skills are contributed at runtime by `src/components/bundled-skills` via `resources_discover` so `disabled_skills` can hide them; a manifest entry loads unfiltered on the launcher `--extension` path), `imports["#omo-task-runtime"] = "./extensions/omo-task.js"` (the dev package maps that specifier to `src/extension/omo-task.ts`). `files` ships extensions, skills, skills-conditional, runtime, `scripts/install.mjs`, README, NOTICE, LICENSE. |
