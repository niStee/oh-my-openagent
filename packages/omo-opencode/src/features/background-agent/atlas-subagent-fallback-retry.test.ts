...
async function launchAtlasOracleSubagent(manager: BackgroundManager): Promise<string> {
  const task = await manager.launch({
    ...
    parentAgent: "atlas",
    model: { providerID: SUPPORTED_PROVIDERS.OPENAI, modelID: SUPPORTED_MODELS.GPT_5_5, variant: "high" },
    fallbackChain: [
      { providers: [SUPPORTED_PROVIDERS.GITHUB_COPILOT], model: "claude-sonnet-4.6", variant: "high" },
    ],
  })
...
describe("Atlas-spawned subagent runtime fallback", () => {
  test("retries oracle subagent on OpenAI usage_limit_reached and registers the fallback session", async () => {
    ...
    expect(task?.model).toEqual({ providerID: SUPPORTED_PROVIDERS.GITHUB_COPILOT, modelID: "claude-sonnet-4.6", variant: "high" })
    expect(task?.attemptCount).toBe(1)
    expect(createdSessions).toHaveLength(2)
    expect(createdSessions[1]?.body?.model).toEqual({ providerID: SUPPORTED_PROVIDERS.GITHUB_COPILOT, id: "claude-sonnet-4.6", variant: "high" })
