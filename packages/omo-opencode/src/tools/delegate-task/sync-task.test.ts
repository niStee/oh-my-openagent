...
describe("executeSyncTask - cleanup on error paths", () => {
  ...
  test("#given fallback chain set #when sendSyncPrompt fails #then retries with next model", async () => {
    ...
    expect(attemptedModels).toEqual([
      { providerID: SUPPORTED_PROVIDERS.ANTHROPIC, modelID: SUPPORTED_MODELS.CLAUDE_OPUS_4_7, variant: "max" },
  ...
  test("#given fallback chain exhausted #when all retries fail #then returns final error", async () => {
    ...
    const fallbackChain = [
      { providers: [SUPPORTED_PROVIDERS.ANTHROPIC], model: SUPPORTED_MODELS.CLAUDE_OPUS_4_7, variant: "max" },
      { providers: [SUPPORTED_PROVIDERS.OPENCODE_GO], model: SUPPORTED_MODELS.KIMI_K2_6 },
      { providers: [SUPPORTED_PROVIDERS.OPENAI], model: SUPPORTED_MODELS.GPT_5_4, variant: "medium" },
    ]
    ...
    expect(attemptedModels).toEqual([
      { providerID: SUPPORTED_PROVIDERS.ANTHROPIC, modelID: SUPPORTED_MODELS.CLAUDE_OPUS_4_7, variant: "max" },
      { providerID: SUPPORTED_PROVIDERS.OPENCODE_GO, modelID: SUPPORTED_MODELS.KIMI_K2_6, variant: undefined },
      { providerID: SUPPORTED_PROVIDERS.OPENAI, modelID: SUPPORTED_MODELS.GPT_5_4, variant: "medium" },
    ])
  })
